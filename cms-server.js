...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/publish' && req.method === 'POST') {
    try {
      execSync('cd /Users/orlandovaldes-scott/orlandovs && /opt/homebrew/bin/hugo && git add -A && git diff --cached --quiet || git commit -m "CMS publish" && git push && CLOUDFLARE_API_TOKEN=cfat_flmNaLNdXRv9QRbnrAQ8Y4CkYYWcH0eXLgCVEOIpd27ee072 npx wrangler pages deploy public --project-name=orlandovs --branch=main --commit-dirty=true', {timeout:60000});
      json(res, {ok:true});
    } catch(e) {
